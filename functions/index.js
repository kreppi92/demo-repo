...
exports.sendEmailReceipt = functions.https.onCall((data, context) => {
  ...
  const btcAmount = bitcoinConverter(parseInt(amount), "satoshi")
    .to("BTC")
    .toString();
  ...
  return jwt.verify(token, jwtToken, function (err, decoded) {
    if (err) {
      ...
      let options = {
        method: "POST",
        url:
          "https://api.growsurf.com/v2/campaign/" +
          campaignKey +
          "/participant",
        headers: {
          Authorization: authorization,
...
exports.updateGrowsurfDepositCompleted = functions.https.onCall(
  (data, context) => {
    const token = data.token;
