...
class Earn extends Component {
  ...
  getEarns = () => {
    ...
    return getEarn({ token: store.get('token') }).then(function (result) {
      if (result.data.success) {
        this.setState({
          earns: result.data.earns.sort((a, b) => (a.order > b.order) ? 1 : -1)
        })
      } 
            ...
            return(
              <div className = {classes.outerBox} key={index}>
                <div className={classes.innerBox}>
                  ...
                  <div className={classes.subtitle}>{earn.preMessaging}<span className={classes.bold}> {earn.satValue} </span>{earn.postMessaging}</div>
                  <Button className={classes.button} size="small" fullWidth variant={'contained'} color="primary" onClick={this.handleClickLink(index)}>
                    Sign Up
                  </Button>
                </div>
              </div>
