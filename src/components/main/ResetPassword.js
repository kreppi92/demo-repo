...
class ResetPassword extends Component {
  ...
  componentDidMount() {
    ...
    if (parsed.reset === "") {
      ...
      isLinkValid({ token: parsed.reset, shouldReset: false }).then(
        function (result) {
    ...
    if (passwordHasError || password2HasError) {
      ...
      if (password === password2) {
        ...
        this.displayFormError(
          ...
          "",
          true,
          "Please ensure that both your password match."
        );
      }
    }
  }
  ...
  updatePassword(newPassword) {
    ...
    if (parsed.reset === "") {
      ...
      var isLinkValid = Firebase.functions().httpsCallable("isLinkValid");
      isLinkValid({ token: parsed.reset, shouldReset: true }).then(
        function (result) {
          if (result.data.success) {
  ...
  resetPassword(newPassword, token) {
    ...
    updatePassword({ password: newPassword, token: token }).then(
      function (result) {
        if (result.data.success) {
          this.setState({
            password: "",
          ...
          this.displaySnackbar(
            "success",
            "Your password has successfully been updated. Transferring to sign in ..."
          );
